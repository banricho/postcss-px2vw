...
var defaultConfig = {
  ...
  vwUnit: 7.5,            // vw unit value (default: 750)
  vwPrecision: 6,         // vw value precision (default: 6)
  forcePxComment: 'px',   // force px comment (default: `px`)
  keepComment: 'no',      // no transform value comment (default: `no`)
  keepAllComment: 'px2vw-disable'
};
...
Px2vw.prototype.generateThree = function (cssText, dpr) {
  dpr = dpr || 2;
  var self = this;
  ...
  var disabled = !firstRule
    || (firstRule.type === 'comment' && firstRule.comment.trim() === config.keepAllComment);
  if (disabled) return cssText;
...
// generate vw version stylesheet
Px2vw.prototype.generatevw = function (cssText) {
  var self = this;
  var config = self.config;
  var astObj = css.parse(cssText);
          ...
          if (nextDeclaration && nextDeclaration.type === 'comment') { // next next declaration is comment
            if (nextDeclaration.comment.trim() === config.forcePxComment) { // force px
              ...
              if (!noDealPx) {
                ...
                declaration.value = self._getCalcValue('vw', declaration.value); // common transform
                declarations.splice(j + 1, 1); // delete corresponding comment
              }
...
// get calculated value of px or vw
Px2vw.prototype._getCalcValue = function (type, value, dpr) {
  var config = this.config;
  var pxGlobalRegExp = new RegExp(pxRegExp.source, 'g');

  function getValue(val) {
    val = parseFloat(val.toFixed(config.vwPrecision)); // control decimal precision of the calculated value
    return val == 0 ? val : val + type;
  }

  return value.replace(pxGlobalRegExp, function ($0, $1) {
    return type === 'px' ? getValue($1 * dpr / config.baseDpr) : getValue($1 / config.vwUnit);
  });
};
